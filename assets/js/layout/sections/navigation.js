$(document).ready(() => {
  ...
  */
  function showNavDropdownsForMobile(clickedElement) {
    const expanded = $(clickedElement).find('span').attr('aria-expanded');
    if (expanded == 'true') {
  ...
  */
  function hideNavDropdownsForMobile(event) {
    ...
    if (!$element.parents().hasClass('nav')) {
      $navDropdownParent
        .find('.nav__dropdown[style="display: block;"]')
        .slideUp();
  ...
  $('.nav .nav__dropdown .nav__dropdown-col > ul .nav__dropdown a')
    .on('mouseover', function() {
      ...
      const $productImage = $this.closest('.nav__dropdown-inner')
        .find('.nav__dropdown-image');
    ...
    .on('mouseleave', function() {
      ...
      const image = $productImage.data('image');
      $productImage.css('background-image', `url(${image})`);
    });
});
